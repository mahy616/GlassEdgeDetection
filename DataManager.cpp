...
void CDataManager::InitVariables()
{
	...
	QStringList header;
	header << QString::fromLocal8Bit("料号") << QString::fromLocal8Bit("检测结果") << QString::fromLocal8Bit("图像路径") << QString::fromLocal8Bit(" 日期");
...
{
	ui.lineEdit_JPN->setEnabled(bEnabled);
	ui.pushButton_Query->setEnabled(bEnabled);
	ui.checkBox_Date->setEnabled(bEnabled);
}

bool CDataManager::InsertData(QString JPN, bool bOK, QString ImagePath,QString &errMsg)
{
	qDebug() << "InsertData";
	QString result = bOK ? "OK" : "NG";
	QString sql = "insert into " + m_DBName + ".dbo.DetectionResult values(NEWID(),\'" + JPN + "\',";
		    sql += "\'" + result + "\',\'" + ImagePath + "\',\'" + QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss") + "\')";
...
void CDataManager::ConnectToDB()
{
	...
	if (!m_bConnected)
	{
		...
		m_db.setDatabaseName(DBName);                            //设置数据源名称  
...
{
	QString JPN = ui.lineEdit_JPN->text();
	QString sql = "select * from " + m_DBName + ".dbo.DetectionResult where JPN like \'%" + JPN + "%\'";
